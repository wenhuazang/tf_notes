...
# =============================================================================#
def load_visda(input_dir, target_label, fr):
    for name in fr:
        img_name, img_label = name.split(' ')
        # print img_name
        if int(img_label) == int(target_label):
            img = Image.open(os.path.join(input_dir, img_name))
            shape = np.array(img).shape
            # img = img.tobytes()
            # yield os.path.join(input_dir, img_name), shape[-1]
            yield img, shape[-1]
        else:
            continue


def img2tfrecords2(output_dir, output_name, input_dir, train, label):
    ...
    f = open(img_list, 'rb')
    for i, img_raw, shape in enumerate(load_visda(input_dir, label, f)):
        if i == 100:
            break
...
flags.DEFINE_string("output_name", "aeroplane", "The directory name to save data")
flags.DEFINE_string("input_dir", "./data/", "The Directory name to input data")
flags.DEFINE_string("input_name", "", "The name to input data[books, dvd, electronics, kitchen]")
...
if __name__ == '__main__':
    ...
    if flag == 1:
        img2tfrecords(FLAGS.output_dir, FLAGS.output_name, FLAGS.input_dir,
                       FLAGS.is_train, pairs[FLAGS.output_name])
    elif flag == 2:
